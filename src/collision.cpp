...
bool Collider::CheckCollision(PhysicsObject* a0, PhysicsObject* b0, sf::Vector2f& direction) {
    ...
    if (dist_squared <= (r1 + r2) * (r1 + r2)) {
        sf::Vector2f normal = p3 / dist;
        direction = normal * ((r1 + r2) - dist); // correct penetration
        return true;
    }
...
void Collider::changeVelocity(PhysicsObject* a, PhysicsObject* b, sf::Vector2f& direction) {
    ...
    // FIX: use normalized direction
    float dist = std::sqrt(dist2);
    sf::Vector2f normal = direction / dist;

    float relVel = dot(v2 - v1, normal);

    // FIX: don't resolve if separating
    if (relVel > 0) return;

    float impulse = (2 * relVel) / (m1 + m2);
    sf::Vector2f impulseVec = normal * impulse;

    // FIX: remove extra mass multiplication mistake
    a->setVelocity(v1 + impulseVec * m2);
    b->setVelocity(v2 - impulseVec * m1);
}